...
async function main() {
    ...
    const responsibilities = await Promise.all(RESPONSIBILITY_DEFS.map((r) => prisma.responsibility.upsert({
        where: { name: r.name },
    ...
    const pick = (names) => names.map((n) => {
        ...
        if (!r)
            throw new Error(`Unknown responsibility: ${n}`);
    ...
    async function syncRole(args, responsibilityNames) {
    ...
    const adminRole = await syncRole({ id: 'platform-admin-role', name: 'Platform Admin', isSystem: true }, RESPONSIBILITY_DEFS
    ...
    const businessOwnerRole = await syncRole({ id: 'business-owner-role', name: 'Business Owner', businessId: demoBusiness.id }, BUSINESS_OWNER);
    const outletRole = await syncRole({ id: 'outlet-admin-role', name: 'Outlet Admin', businessId: demoBusiness.id }, OUTLET_FULL);
    ...
    const demoOutlet = await prisma.outlet.upsert({
        ...
        create: {
            ...
            businessId: demoBusiness.id,
            outletType: client_1.OutletType.DINE_IN_POSTPAID,
    ...
    const kitchenRole = await syncRole({ id: 'kitchen-manager-role', name: 'Kitchen Manager', businessId: demoBusiness.id }, KITCHEN_MANAGER);
    const cashierRole = await syncRole({ id: 'cashier-role', name: 'Cashier', businessId: demoBusiness.id }, CASHIER);
    const storeRole = await syncRole({ id: 'store-manager-role', name: 'Store Manager', businessId: demoBusiness.id }, STORE_MANAGER);
